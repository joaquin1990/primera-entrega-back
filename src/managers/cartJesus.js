  ...
  addNewCart: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({
          description: `new cart successfully created with id=${newCart.id}`,
          data: newCart,
        });
    } catch (error) {
      console.warn({
        class: `cartsController`,
        method: `addNewCart: async (req, res)`,
        description: error,
      });
      res
        .status(500)
    ...
    try {
      ...
      if (!cartFound) {
        ...
        res
          .status(200)
          .json({
            description: `The cart with id=${cartId} has been removed.`,
          });
      }
    } catch (error) {
      console.warn({
        class: `cartsController`,
        method: `deleteCartById: async (req, res)`,
        description: error,
      });
      res
        .status(500)
  ...
  getAllProductListByCartId: async (req, res) => {
    try {
      ...
      if (!cartFound) {
        ...
        return res
          .status(200)
          .json({ description: `Cart found, content 0 products.`, data: [] });
      } else {
        cartFound.products = cartFound.products.map((item) => {
          let productItem = productFound.find(
          ...
          return productItem
            ? { ...editproductItem, quantity: item.quantity }
            : item;
        });
        return res
          .status(200)
          .json({
            description: `Cart found, content ${cartFound.products.length} products.`,
            data: cartFound.products,
          });
      }
    } catch (error) {
      console.warn({
        class: `cartsController`,
        method: `getAllProductListToByCartId: async (req, res)`,
        description: error,
      });
      res
        .status(500)
  ...
  addProductToCart: async (req, res) => {
    try {
      ...
      if (!productFound) {
        return res
          .status(422)
          .json({ description: `Product ${pId} not found.` });
      } else if (pquantity < 1) {
        return res
          .status(422)
          .json({ description: "Quantity must be greater than 0." });
      } else if (productFound.stock == 0) {
        return res
          .status(422)
          .json({ description: `Product stock not found.` });
      ...
      const ProductItemInCarts = productsInCartsFound.find(
      ...
      if (!ProductItemInCarts) {
        ...
        return res
          .status(200)
          .json({
            description: `Product ${pId} added to cart ${cId} successfully.`,
            data: cartFound,
          });
      } else {
        //Update quantity
        ...
        cartFound.products = cartFound.products.map((item) =>
          item.id !== pId
            ? item
            : { ...item, quantity: item.quantity + pquantity }
        );
        await cartDB.updateById(cartFound);
        ...
        return res
          .status(200)
          .json({
            description: `(${pquantity}) unid(s) of (${productFound.id}) - ${productFound.name} added successfully in Cart.`,
            data: cartFound,
          });
      }
    } catch (error) {
      console.warn({
        class: `cartsController`,
        method: `addProductToCart: async (req, res)`,
        description: error,
      });
      res
        .status(500)
    ...
    try {
      ...
      if (!productFound) {
        ...
        return res
          .status(200)
          .json({
            description: `Producto : (${productFound.id}) - ${productFound.name} was removed from your cart.`,
            data: cartFound,
          });
      }
    } catch (error) {
      console.warn({
        class: `cartsController`,
        method: `deleteProductToCartById: async (req, res)`,
        description: error,
      });
      res
        .status(500)
